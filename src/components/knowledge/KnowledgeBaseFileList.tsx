...
export const KnowledgeBaseFileList: React.FC<KnowledgeBaseFileListProps> = ({
  ...
  const filteredFiles = sortedFiles.filter(file => {
    const matchesSearch = file.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                        ...
                        (file.tags?.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())) ?? false);
    const matchesCategory = categoryFilter ? file.category === categoryFilter : true;
    return matchesSearch && matchesCategory;
  });

  const SortButton = ({ field, label }: { field: keyof KBFile; label: string; }) => (
    <Button 
      variant="ghost" 
      size="sm" 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center text-sm text-slate-500 mt-4">
        ...
        {searchTerm && (
          <Button variant="ghost" size="sm" onClick={() => setSearchTerm('')} className="h-8 text-xs">
            Clear Search
          </Button>
        )}
      </div>
